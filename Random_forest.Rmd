...
```{r}
set.seed(123) # set the seed so that the outcome can be repeatable
```

## Without normalization
We build a random forest with the unnormalized attributes
```{r}
rand_forest <- randomForest(Class ~ Area +
                              MajorAxisLength +
                              MinorAxisLength +
                              Eccentricity +
                              ConvexArea +
...
predict_train_data <- predict(rand_forest, train_data)
confusionMatrix(predict_train_data, train_data$Class)
```

```{r}
# prediction with test data
predict_test_data <- predict(rand_forest, test_data)
...
We build a random forest with the z-normalized attributes
```{r}
rand_forest_z_normalization <- randomForest(Class ~ z_transformed_Area +
                                            ...
                                            data=train_data,
                                            proximity=TRUE)
print(rand_forest_z_normalization)
```

```{r}
# prediction with training data
predict_train_z_normalization <- predict(rand_forest_z_normalization, train_data)
confusionMatrix(predict_train_z_normalization, train_data$Class)
```

```{r}
# prediction with test data
predict_test_data_z_normalization <- predict(rand_forest_z_normalization, test_data)
...
We build a random forest with the z-log normalized attributes
```{r}
rand_forest_z_log_normalization <- randomForest(Class ~ z_log_transformed_Area +
                                                ...
                                                data=train_data,
                                                proximity=TRUE)
print(rand_forest_z_log_normalization)
```

```{r}
# prediction with training data
predict_train_z_log_normalization <- predict(rand_forest_z_log_normalization, train_data)
confusionMatrix(predict_train_z_log_normalization, train_data$Class)
```

```{r}
# prediction with test data
predict_test_data_z_Log_normalization <- predict(rand_forest_z_log_normalization, test_data)
...
error_list <- c()
for (i in 1:K){
  model <- randomForest(Class ~ z_transformed_Area +
                          z_transformed_MajorAxisLength +
                          z_transformed_MinorAxisLength +
                          z_transformed_Eccentricity +
                          z_transformed_ConvexArea +
...
error_list <- c()
for (i in 1:K){
  model <- randomForest(Class ~ z_log_transformed_Area +
                          z_log_transformed_MajorAxisLength +
                          z_log_transformed_MinorAxisLength +
                          z_log_transformed_Eccentricity +
                          z_log_transformed_ConvexArea +
...
```{r}
write.csv2(df, file="./dataset/results/random_forest.csv", row.names = FALSE)
```
